...
class StableEmbeddingModelViT(nn.Module):
    ...
    def __init__(self,
        ...
        print(
            f"StableEmbeddingModel initialized with ViT backbone: {backbone_model_name if not custom_backbone else 'custom'}")
    ...
    def __init__(self,
        ...
        if custom_backbone:
            ...
            if not backbone_out_features == 768:  # Check for convnext_tiny
                print(
                    f"Warning: backbone_out_features is {backbone_out_features}, but convnext_tiny typically outputs 768 features. Correcting to 768.")
                self.backbone_out_features = 768
        else:
            raise ValueError(
                f"Unsupported backbone_model_name: {backbone_model_name}. Provide 'convnext_tiny' or a custom_backbone.")
        ...
        print(
            f"StableEmbeddingModel initialized with backbone: {backbone_model_name if not custom_backbone else 'custom'}")
        print(f"  Embedding Dim: {embedding_dim}, Num Classes: {num_classes}")
        print(f"  ArcFace s: {arcface_s}, m: {arcface_m}")
        ...
        try:
            ...
            if hasattr(self.backbone, 'features') and isinstance(self.backbone.features, nn.Sequential) and len(
                    self.backbone.features) > 0:
                if isinstance(self.backbone.features[0], nn.ModuleList) and len(
                        self.backbone.features[0]) > 0:  # ConvNeXt structure
            ...
            elif hasattr(self.backbone, 'stem') and hasattr(self.backbone.stem, '0') and isinstance(
                    self.backbone.stem[0], nn.Conv2d):  # Timm models often have a 'stem'
